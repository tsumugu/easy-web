...
function is_this_havenot_value($str) {
  if (empty($str)) {
    return false;
  }
  return mb_substr_count($str,":") === 1 && mb_substr_count($str,",") === 0 && mb_substr($str, -1) === ":";
}
function is_this_havenot_parent($str) {
  if (empty($str)) {
    return false;
  }
...
function make_nest_text($notfound_array) {
  ...
  foreach ($notfound_array as $tmp_k=>$tmp_nf) {
    if (is_this_havenot_value($tmp_nf) && !is_this_havenot_value($notfound_array[$tmp_k+1])) {
      $tmp_c_text = "";
      ...
      //for ($i=$tmp_k+1; count($notfound_array)>$i; $i++) {
        ...
        if (!is_this_havenot_parent($tmp_notfound_array_text) && !is_this_havenot_value($tmp_notfound_array_text)) {
          $tmp_c_text .= "[".$tmp_notfound_array_text."]";
        } else {
          $tmp_c_text .= $tmp_notfound_array_text;
        }
        unset($notfound_array[$i]);
        if (is_this_havenot_parent($tmp_notfound_array_text)) {
          break;
        }
      }
...
function generate_have_child_com_array($notfound_array) {
  ...
  foreach ($notfound_array as $nf_val_arr) {
    ...
    foreach ($nest_replace_array as $nest_content) {
      ...
      $tmp_rep_text = replace_and_generate_html($nest_content[1], $tmp_components_array);
      $tmp_components_array_2 = generate_components_array(array("[".$tmp_rep_text."]"));
      $tmp_rep_text_2 = replace_and_generate_html("[".$tmp_rep_text."]", $tmp_components_array_2);
    ...
    $tmp_rep_nest_replaced_fin_str = replace_and_generate_html("[".$nest_replaced_fin_str."]", $tmp_nest_replaced_fin_str_gen);
...
function replace_and_set_template($str, $project_dir) {
  $ret = null;
...
function replace_and_generate_html($str, $components_array) {
  foreach ($components_array as $type=>$component) {
    ...
    if ($type === "func") {
      ...
      // no value
      if ($value === "-" || $value === "-nv" || $value === "-novalue" || $value === "-no-value") {
...
function analyze_texts($str, $project_dir) {
    ...
    if (count_brackets_pair($tmp_hcm_c)) {
      ...
      if(preg_match('/(\[|\])/',$tmp_hcm_c)){
        ...
        foreach ($tmp_exp_hcm as $tmp_tec) {
          //
          $rep_and_set_template = replace_and_set_template($tmp_tec, $project_dir);
        ...
        //
        $rep_and_set_template = replace_and_set_template($tmp_hcm_c, $project_dir);
        if (is_null($rep_and_set_template)) {
  ...
  $str = replace_and_generate_html($str, $components_array);
  //debug
...
function replace_strc($project_dir, $strc_array, $contents_array) {
  $ret_str = "";
...
function generate_html($project_dir, $strc_file_path, $contents_file_path) {
  ...
  $strc_replaced = replace_strc($project_dir, $analyzed_strc_file_lines, $analyzed_contents_file_lines);
